"""Topic-level citation-retry reliability (Stage 4).

A drafting failure caused ONLY by citation validation (a hallucinated key, or a
required section with no [@key]) is usually just LLM variance, so the pipeline
redraws the whole topic up to CITATION_REDRAFT_RETRIES times before failing
closed. Any other validation failure fails immediately, and a persistent
citation failure still fails closed — validation is never loosened. All offline.
"""
...
from proposal_engine.pipeline import CITATION_REDRAFT_RETRIES, run_topic
...
def test_citation_failure_retries_up_to_the_cap_then_fails_closed(tmp_path, fake_llm, monkeypatch):
    ...
    assert calls["n"] == CITATION_REDRAFT_RETRIES + 1
